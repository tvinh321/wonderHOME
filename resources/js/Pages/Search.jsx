...
export default function Search() {
    ...
    const [price, setPrice] = useState(searchParams.get("price") ? searchParams.get("price").split('-') : null);
    const [area, setArea] = useState(searchParams.get("area") ? searchParams.get("area").split('-') : null);
    const [type, setType] = useState(searchParams.get("type") ? searchParams.get("type").split(',') : null);
    ...
    return (
        <>
            ...
            <div className="border-y mt-6 py-2 hidden md:block">
                <div className="grid lg:grid-cols-10 grid-cols-5 mx-24 text-sm font-semibold">
                    ...
                    <div className="col-span-1 px-4 py-2">
                        ...
                        <select className="px-1 py-2 w-full rounded-md bg-gray-100 border-transparent focus:border-gray-500 focus:bg-white focus:ring-0 text-xs" value={price}
                                onChange={(e) => {
                    ...
                    <div className="col-span-1 px-4 py-2">
                        <p className="mb-2">Diện tích</p>
                        <select className="px-1 py-2 w-full rounded-md bg-gray-100 border-transparent focus:border-gray-500 focus:bg-white focus:ring-0 text-xs"
                            onChange={(e) => {
                    ...
                    <div className="col-span-1 px-4 py-2 flex items-center justify-center">
                        <button className="flex items-center justify-center w-full h-10 bg-amber-400 rounded text-white font-semibold">Tìm kiếm</button>
                    </div>
                </div>
            </div>

            <div className="mt-10 mx-56 mb-16 grid grid-cols-4">
                <div className="col-span-3">
                    <p className="font-semibold mb-10">Hiện tổng có {searchResults?.length} bất động sản được lọc</p>
                    {searchResults
                        ? searchResults.map((item, index) => {
                                return (
                                    <div className="flex items-center justify-between mb-10" key={index}>
                                        <div className="flex">
                                            <a href={`/thong-tin/${item.id}`}><img src={index % 6 === 0 ? Room1 : index % 6 === 1 ? Room2 : index % 6 === 2 ? Room3 : index % 6 === 3 ? Room4 : index % 6 === 4 ? Room5 : Room6} alt="" className=" w-56 h-36 object-cover" />
                                            </a>
                                            <div className="ml-4">
