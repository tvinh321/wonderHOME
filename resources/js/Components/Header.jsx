        ...
        <header>
            <nav>
                ...
                <div className="relative flex h-16 items-center justify-center">
                    ...
                    <div className="hidden md:ml-6 md:block">
                        <div className="flex space-x-4">
                        {
                            NAVLINK_VIE.map((value, index) => {
                                return <a href={SERVICES[index]} className="text-neutral-900 hover:bg-neutral-900 hover:text-white block px-3 py-2 rounded-md text-base font-medium transition-all duration-150" key={index}>{value}</a>
                            })
                        }
                        </div>
                    </div>
                    ...
                    <a href="/">
                        <img className="inline-block w-[200px]" src="/assets/images/logo.png" alt="wonderHOME" />
                    </a>  
                    </div>
                    <div className="absolute inset-y-0 right-0 flex items-center pr-2 md:static md:inset-auto md:ml-4 md:pr-0">
                    <button type="button" className="hidden md:inline rounded-full p-1 mr-4 hover:text-amber-400 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800 transition-all duration-150">
                    ...
                    <button className="hidden md:flex group mr-4 text-amber-300 border border-amber-300 hover:border-transparent hover:bg-amber-300 font-bold py-2 px-4 rounded items-center">
                        <span className="text-sm group-hover:text-white mr-1 transition-all duration-150">Đăng tin</span>
                        <svg className="transition-all duration-150 fill-amber-300 group-hover:fill-white" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><path d="M17,19.22H5V7h7V5H5C3.9,5,3,5.9,3,7v12c0,1.1,0.9,2,2,2h12c1.1,0,2-0.9,2-2v-7h-2V19.22z"/><path d="M19,2h-2v3h-3c0.01,0.01,0,2,0,2h3v2.99c0.01,0.01,2,0,2,0V7h3V5h-3V2z"/><rect height="2" width="8" x="7" y="9"/><polygon points="7,12 7,14 15,14 15,12 12,12"/><rect height="2" width="8" x="7" y="15"/></svg>
                    </button>
                    {
                        isLoggedIn ? (
                            <div className="relative ml-3">
                                ...
                                <div className="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
                                    <a href="#" className="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="user-menu-item-0">Cá nhân</a>
                                    <a href="#" className="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="user-menu-item-1">Cài đặt</a>
                                    <a href="#" className="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="user-menu-item-2">Thoát</a>
                                </div>
                            </div>
                        ) : (
                            <a href="/dang-nhap">
                                <button className="group mr-4 bg-amber-300 text-neutral-900 hover:bg-amber-300 font-bold py-2 px-4 rounded inline-flex items-center">
                                    <span className="hidden md:inline text-sm group-hover:text-white mr-1 transition-all duration-150">Đăng ký / Đăng nhập</span>
                                    <svg className="fill-neutral-900 group-hover:fill-white transition-all duration-150" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
                                        <g>
                ...
                <div className="space-y-1 px-2 pt-2 pb-3">
                    {
                        NAVLINK_VIE.map((item, index) => (
                            <a href={SERVICES[index]} className="text-neutral-900 hover:bg-neutral-900 hover:text-white block px-3 py-2 rounded-md text-base font-medium" key={index}>{item}</a>
                        ))
                    }
                </div>
                </div>
